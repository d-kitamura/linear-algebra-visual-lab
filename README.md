...
フェーズ 3「2D 教材版の完成」まで実装・利用者確認・統合棚卸しが完了しています。現在は新設したフェーズ 4「2D 一次結合エクスプローラ」を進めており、4.1〜4.3の求解・共有・ターゲット操作に続き、4.4として不能・唯一解・不定の分類、係数例、一般解、一般のターゲットに対する係数公式、原点基準の平行四辺形を実装しました。D-034では表示ベクトルを `a₁`〜`a₈`、選択集合を `S`、列行列を `A`、ターゲットを `v` へ統一し、不定の2例と導出説明付き一般解へ表示を改訂しています。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。授業シナリオと統合確認は4.5、3D教材版はフェーズ 5で実装します。
