...
14.8で[内積・正規直交基底Labの授業資料](./INNER_PRODUCT_TEACHING_GUIDE.md)に18例を追加し、7Lab・計106代表例となる（D-133確認待ち）。以下の既存88例の入力・期待値・URLは変更しない。共有機能のD-132は確認待ちを維持する。
...
13.7で[対角化Labの授業資料](./DIAGONALIZATION_TEACHING_GUIDE.md)に18例を追加し、13.7時点で6Lab・計88代表例。既存70例の内容・URLは不変。新18例は独立期待値・共有復元・Resetを回帰検証し、D-123は利用者確認済み。13.8の[6Lab統合棚卸し](./INVENTORY_PHASE13.md)もD-124承認済みでフェーズ13完了。
...
| 例 | 行列・次元 | rank | nullity | 単射 | 全射 | 観察点 |
|---|---|---:|---:|---|---|---|
| 恒等写像 | `2→2`, `[[1,0],[0,1]]` | 2 | 0 | 成立 | 成立 | すべての入力を変えず、核は原点、像は終域全体 |
| 90°回転 | `2→2`, `[[0,-1],[1,0]]` | 2 | 0 | 成立 | 成立 | 形を回転しても次元を失わず全単射 |
| x軸に関する鏡映 | `2→2`, `[[1,0],[0,-1]]` | 2 | 0 | 成立 | 成立 | 向きを反転しても核は原点だけ |
| せん断 | `2→2`, `[[1,1],[0,1]]` | 2 | 0 | 成立 | 成立 | 格子の形は変わってもrankを保つ |
| x軸へのrank 1射影 | `2→2`, `[[1,0],[0,0]]` | 1 | 1 | 不成立 | 不成立 | 1方向が核へ失われ、像は直線になる |
| 零写像 | `2→2`, `[[0,0],[0,0]]` | 0 | 2 | 不成立 | 不成立 | 定義域全体が核、像は原点だけ |
| 2次元から3次元への埋め込み | `2→3`, `[[1,0],[0,1],[0,0]]` | 2 | 0 | 成立 | 不成立 | 核は原点だが、像は3次元内の平面 |
| 3次元から2次元への射影 | `3→2`, `[[1,0,0],[0,1,0]]` | 2 | 1 | 不成立 | 成立 | 1方向を失いながら終域全体へ写す |

各例では「行列と入力」「行列の列・核・像」「写像の線形性」「次元定理」を順に切り替え、`dim(U)=rank(T)+null(T)`と単射・全射の条件を照合する。線形写像Labで共有URLまたはQRコードを作成すると、現在の行列、`u,w,c`、格子表示、3Dカメラが保存される。別端末で開いて操作した後のResetは配布時状態へ戻る。
