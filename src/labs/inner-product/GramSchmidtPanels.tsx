...
import { InnerObjectValue, InnerPolynomialSpace } from './innerProductPolynomialMath';
import { Fraction, Norm, NumberValue, Product } from './InnerProductPanels';
...
export function GramSchmidtStepPanel({ analysis, stage }: { readonly analysis: GramSchmidtAnalysis; readonly stage: StageKey | null }) {
  ...
  const a = `a${input.id}`;
  const polynomial = analysis.definition.metric !== 'euclidean';
  return <>
    <p>{step.inputPosition}番目の入力：<Vector name={a} /> — {stageLabel(stage)}</p>
    <Formula><Vector name={a} /> = <InnerObjectValue polynomial={polynomial} value={{ status: 'ready', value: input.components }} /></Formula>
    ...
    {projections.map((projection, index) => {
      ...
      return <div className="inner-step-projection" key={projection.ontoSourceId}>
        ...
        <Formula><Vector name={w} /> = <InnerObjectValue polynomial={polynomial} value={accepted.w.numeric} /></Formula>
        <Formula><Vector name={p} /> = <Fraction top={<Product left={a} right={w} />} bottom={<Product left={w} right={w} />} /><Vector name={w} />
          = (<NumberValue value={projection.coefficient.numeric} />)<Vector name={w} /> = <InnerObjectValue polynomial={polynomial} value={projection.vector.numeric} /></Formula>
      </div>;
    })}
    {residual && <Formula><Vector name="r" /> = <Vector name={a} />{projections.map((_, index) => <span className="representation-atom" key={index}> − <Vector name={`p${index + 1}`} /></span>)} = <InnerObjectValue polynomial={polynomial} value={residual.numeric} /></Formula>}
    ...
    {stage.phase === 'normalize' && step.outputIndex !== null && step.q && <>
      <Formula><Vector name={`w${step.outputIndex}`} /> = <Vector name="r" /> = <InnerObjectValue polynomial={polynomial} value={step.residual?.numeric ?? null} /></Formula>
      <Formula><Norm name={`w${step.outputIndex}`} /> = <NumberValue value={step.norm} /></Formula>
      <Formula><Vector name={`q${step.outputIndex}`} /> = <Fraction top={<Vector name={`w${step.outputIndex}`} />} bottom={<Norm name={`w${step.outputIndex}`} />} /> = <InnerObjectValue polynomial={polynomial} value={{ status: 'ready', value: step.q }} /></Formula>
...
export function GramSchmidtBasisPanel({ analysis }: { readonly analysis: GramSchmidtAnalysis }) {
  ...
  return <>
    ...
    <Formula><Scalar>W</Scalar> = <span className="representation-atom">span(<Scalar>S</Scalar>)</span><span>、</span><Scalar>V</Scalar> = {analysis.definition.metric === 'euclidean' ? <span>ℝ<sup>{analysis.definition.dimension}</sup></span> : <InnerPolynomialSpace dimension={analysis.definition.dimension} />}</Formula>
    ...
    {analysis.accepted.map(item => <div className="inner-basis-item" key={item.sourceId}>
      <Formula><Vector name={`q${item.outputIndex}`} /> = <InnerObjectValue polynomial={analysis.definition.metric !== 'euclidean'} value={{ status: 'ready', value: item.q }} /></Formula><p><Vector name={`a${item.sourceId}`} />から採用</p>
    </div>)}
