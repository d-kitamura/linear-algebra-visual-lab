...
この文書は、現在のロードマップでは作業単位6.1に当たる初回公開の実装記録と利用者向け操作手順である。GitHub Actionsワークフロー、Pages設定、初回公開、本番環境での共有URL・Reset等の確認まで完了している。
...
初回公開後は、Codexが作業単位ごとに差分、テスト、型検査、公開用ビルドを確認し、既存履歴と同じ短い日本語のメッセージでmainへcommit・pushする。同じワークフローが自動実行され、成功後に公開版が更新される。Codexは利用可能な範囲で対象commitのActions完了と公開URLを確認し、commit SHA、push、デプロイの結果を利用者へ報告する。
