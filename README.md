...
内積・正規直交基底Labは14.1〜14.9と最終確認D-134まで承認済みで、フェーズ14は完了です。[採用設計](docs/INNER_PRODUCT_LAB_DESIGN.md)・[API契約](docs/INNER_PRODUCT_API_CONTRACT.md)・[ロードマップ](ROADMAP.md)を参照してください。
...
フェーズ0〜6・8〜14は完了し、合意した7Labの単元実装を終えています。フェーズ7の授業評価は延期しています。現行は初版候補 `1.0.0-rc.1` で、正式リリースと共有URL互換保証は未開始です。ケイリー・ハミルトンの定理・次数落としは利用者指示により対象外です。
