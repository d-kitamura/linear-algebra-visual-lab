...
2026-09-22更新。**D-125・D-126・D-127は利用者承認済み。14.3・14.4（D-128・D-129）承認済み、14.5・D-130承認済み、14.6多項式1〜3D実装済み・D-131確認待ち。** 共有デコーダーは未実装（14.7予定）。14.2〜14.6の実装記録は第10〜14節を参照。実画面確認後の見直しを認める。
...
D-127は2026-09-21に承認済み。数2D画面は14.3、GS操作は14.4、数0〜3Dは14.5、多項式1〜3Dは14.6で接続した（第11〜14節）。次の14.7は段階の意味的復元と既存共有境界を保つため**「高」推奨**。
...
2026-09-22。D-130承認済み。以下は14.5時点の記録。数ベクトルの4場面を実装。数学APIの判定・演算予算は不変。多項式は14.6で接続、共有14.7は未接続。
...
## 14. 14.6多項式と内積対応座標（D-131確認待ち）

2026-09-22。7場面と2つの多項式内積を接続。14.2数学APIの計算・判定・予算は変更しない。共有14.7は未接続。
...
- 回帰は独立期待値（定数1、1とx²、(1,x,x²)のGS）、7場面、表示の内積／直角、逆変換・上限・非零消失、Reset・段階保持・表記を対象とする。実機操作はD-131で確認する。
