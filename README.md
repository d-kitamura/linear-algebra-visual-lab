...
現在は要件整理と開発計画の段階です。実装は小さな単位に分け、各段階で実際の動作を確認してから次へ進めます。Codex はローカルファイルのみを変更し、コミットとプッシュはプロジェクト所有者が行います。
...
- [`docs/PROJECT_STATUS.md`](./docs/PROJECT_STATUS.md): 現在地と次に行う小さな作業
