...
describe('フェーズ13の未承認設計と利用者指定の範囲', () => {
  it('全体方針の承認と13.1具体契約の確認待ちを区別する', () => {
    ...
    expect(contract).toContain('利用者確認待ち（D-116）');
    expect(contract.match(/- \[x\]/g)).toHaveLength(3);
    expect(contract).toContain('- [ ] **確認ゲート:**');
    expect(phase13.split('### 13.2 ')[1]).not.toContain('- [x]');
