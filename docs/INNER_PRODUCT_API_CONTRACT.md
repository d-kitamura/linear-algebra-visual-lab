...
2026-09-21更新。**D-125・D-126・D-127は利用者承認済み。14.3数2D画面実装済み・D-128確認待ち。** 共有デコーダーは未実装（14.7予定）。14.2・14.3の実装記録は第10・11節を参照。実画面確認後の見直しを認める。
...
D-127は2026-09-21に承認済み。数2D画面は14.3で接続した。次の14.4は確定したGSの計算過程を段階表示へ接続するため**「中」推奨**。

## 11. 14.3画面接続（D-128確認待ち）

2026-09-21。数2D・固定2本のみ実装。`innerProductScene.ts`が入力・pair参照・補助図設定を保持し、`InnerProductLab.tsx`が下書き・表示範囲・ドラッグpreview・タブを分離する。共有形式は未接続（14.7）。GSの解析／段階操作は14.4で接続する。
