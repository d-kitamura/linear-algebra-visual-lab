...
- 現行の生成形式はLabごとに分かれ、ベクトル空間Labは`vector-space` v3、基底・次元Labは`basis-dimension` v1、線形写像Labは`linear-map` v1である。
- 現行3形式が扱うのは2D/3Dである。ベクトル空間Labと基底・次元Labの0D/1Dは実装・確認済みだが、共有は10.7の版更新まで無効である。
- 3形式は最初の正式リリースで保証対象になる候補として固定fixtureを用意している。
...
1. `vector-space` v3、`basis-dimension` v1、`linear-map` v1を最初の保証対象候補とし、正式承認後は本アプリを提供している期間、固定の年数を設けず読み込みを維持する。
...
| `linear-map` v1 | 現行 | 維持 | 正式リリース承認後、期限なし | 入出力次元、行列、2入力、スカラー、格子表示、両側3Dカメラを保持 |
