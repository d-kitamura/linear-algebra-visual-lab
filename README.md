...
フェーズ6「共有と配布の仕上げ」とフェーズ8「複数Lab基盤と基底・次元Lab」の実装・利用者確認・統合棚卸しが完了しています。2D/3D切替、右手座標系・正投影カメラ、数値編集・解析・共有、rankに応じたspan幾何、一次結合、モードなしの矢先ドラッグ、平行・同一平面上への吸着、spanと係数幾何のドラッグ中更新を利用できます。現在は初版候補 `1.0.0-rc.1` で、授業実施までフェーズ7の評価を延期しています。ベクトル空間Labと基底・次元Labに加え、シラバス第9〜10週へ接続するフェーズ9「線形写像Lab」を実装中です。9.6までの定義域／終域連動、行列編集、核・像、rank・nullity、線形性、次元定理、`linear-map` v1共有URL・QR・共有時Reset、8代表例、キーボード操作を利用者確認済みとしました。9.7では3つのLabの状態分離、3種類の共有状態、計28代表例、依存関係、数式表記、後続の表現行列Labとの責務境界を統合棚卸し済みで、利用者確認を待っています。3Dカメラ、固定fixture、授業手順、Canvas代替導線、MIT License、利用・プライバシー案内を維持しています。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
