...
フェーズ 4「2D 一次結合エクスプローラ」までの実装・利用者確認・統合棚卸しと、共有URLのQR表示・PNG保存が完了しています。現在はフェーズ5「3D教材版」を進行中です。5.1〜5.5で次元切替、右手座標系・正投影カメラ、3D数値編集・解析・共有、rank 0〜3のspan幾何表示、3D一次結合を利用者確認済みです。5.6では矢先ドラッグを画面に平行な面内のベクトル移動、背景ドラッグを視点回転としてモードなしで自動分離し、他のベクトルとの平行・同一平面上へ画面相対の距離で吸着する3D直接編集を実装しました。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
