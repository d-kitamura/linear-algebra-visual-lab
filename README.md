...
フェーズ6「共有と配布の仕上げ」、フェーズ8「複数Lab基盤と基底・次元Lab」、フェーズ9「線形写像Lab」の実装・利用者確認・統合棚卸しが完了しています。2D/3D切替、右手座標系・正投影カメラ、数値編集・解析・共有、rankに応じたspan幾何、一次結合、基底・座標、多項式係数表示、定義域／終域連動、核・像、rank・nullity、線形性、次元定理を利用できます。現在は初版候補 `1.0.0-rc.1` で、授業実施までフェーズ7の評価を延期しています。3つのLabの共有状態、3Dカメラ、固定fixture、計28代表例、授業手順、Canvas代替導線、MIT License、利用・プライバシー案内を維持しています。フェーズ10「0・1次元空間の基礎拡張」は10.1・10.2の確認を終え、10.3で共通1D数直線と0D一点表示を実装しました。既存3Labの次元切替と共有URLはまだ2D/3Dのままで、10.4以降に新しい共通図へ接続します。その後のフェーズ11には数ベクトルと多項式空間を扱う第四の「表現行列・基底変換Lab」を計画しています。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
