...
フェーズ 4「2D 一次結合エクスプローラ」まで実装・利用者確認・統合棚卸しが完了しています。不能・唯一解・不定の分類、係数例、一般解、一般のターゲットに対する係数公式、原点基準の平行四辺形を、ターゲットを含む共有URLとResetへ統合しました。表示ベクトルは `a₁`〜`a₈`、選択集合は `S`、列行列は `A`、ターゲットは `v` に統一し、不定では異なる2例と導出説明付き一般解を表示します。授業用には1本、一次従属な2本、一次独立な2本、3本の共有可能な代表例を用意しています。フェーズ5前の追加作業4.6では、共有URLをブラウザ内でQRコード化し、ダイアログ表示とPNG保存へ対応しました。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。次の3D教材版はフェーズ 5で検討・実装します。
