...
フェーズ6「共有と配布の仕上げ」までの実装・利用者確認・統合棚卸しが完了しています。2D/3D切替、右手座標系・正投影カメラ、数値編集・解析・共有、rankに応じたspan幾何、一次結合、モードなしの矢先ドラッグ、平行・同一平面上への吸着、spanと係数幾何のドラッグ中更新を利用できます。3Dの直接操作では通常ベクトルと一次結合ターゲットの吸着距離を表示幅の3%、2Dは1%とし、通常ベクトルは原点吸着を平行・同一平面吸着より優先します。2D・3D合わせて16件の代表例、現行v3の本番共有URL、QR表示・PNG保存、授業手順、Canvasを利用できない場合の代替導線、MIT License、利用・プライバシー案内も整備済みです。現在は初版候補 `1.0.0-rc.1` です。授業実施までフェーズ7の評価を延期し、フェーズ8「複数Lab基盤と基底・次元Lab」を進めています。8.4cまで利用者確認済みです。8.5では2D/3Dのターゲット操作、一意な座標、非基底時の3分類、同じターゲットに対する別基底・別順序の座標比較を追加しました。8.5aでは成分・判定・座標を既存Labと同じ詳細タブへまとめ、列ベクトルの角括弧と係数`\bm{c}`の表記を統一しました。現在は8.5・8.5aの利用者確認待ちで、数ベクトル版の採否確認は8.6、基底・次元Labの共有URL・QRは8.7で扱います。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
