...
export function Tuple({ basis, mapped = false, renderElement }: { readonly basis: VectorSet; readonly mapped?: boolean; readonly renderElement?: (name: string) => ReactNode }) {
  return <span className="representation-atom">({basis.vectors.map((v, i) => <span key={v.id}>{i > 0 && ', '}{renderElement ? renderElement(v.name) : mapped ? <MapValue name={v.name} /> : <Vector name={v.name} />}</span>)})</span>;
}
...
export function CoordinateName({ basis, mapped = false, object }: { readonly basis: 'B' | 'C'; readonly mapped?: boolean; readonly object?: ReactNode }) {
  return <span className="representation-atom">[{object ?? (mapped ? <MapValue name="w" /> : <Vector name="w" />)}]<sub><BasisName name={basis} /></sub></span>;
}
...
export function Combination({ basis, coefficients, renderElement }: { readonly basis: VectorSet; readonly coefficients: readonly number[]; readonly renderElement?: (name: string) => ReactNode }) {
  return <>{basis.vectors.map((v, i) => <span className="representation-atom" key={v.id}>{i > 0 && ' + '}({formatMathNumber(coefficients[i]).text}){renderElement ? renderElement(v.name) : <Vector name={v.name} />}</span>)}</>;
}
