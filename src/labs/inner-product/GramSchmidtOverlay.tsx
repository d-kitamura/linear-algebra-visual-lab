...
export function GramSchmidtOverlay({ analysis, stage, viewport }: {
  readonly analysis: GramSchmidtAnalysis; readonly stage: StageKey | null; readonly viewport: PlaneViewport;
}) {
  ...
  if (frame.projections.length === 1) {
    ...
    return <ProjectionGeometry u={direction.value} v={input.components} p={first.vector.numeric.value} r={frame.residual.numeric.value} viewport={viewport} />;
  }
  // 2Dで2方向を引き切った厳密零の場合は、2つの射影を辺にした補助図。
  if (second?.vector.numeric.status === 'ready' && frame.residual.exact.every(x => x.numerator === 0n)) {
    return <ProjectionGeometry u={first.vector.numeric.value} v={input.components} p={first.vector.numeric.value} r={second.vector.numeric.value} viewport={viewport} />;
  }
  return null;
}
