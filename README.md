...
フェーズ 4「2D 一次結合エクスプローラ」までの実装・利用者確認・統合棚卸しと、共有URLのQR表示・PNG保存が完了しています。現在はフェーズ5「3D教材版」を進行中です。5.1の次元切替と5.2の右手座標系・正投影カメラは利用者確認済みで、5.3では0〜8本の3D数値編集、span選択、rank・次元・一次独立性の解析、共有状態v3によるカメラ復元を実装しました。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
