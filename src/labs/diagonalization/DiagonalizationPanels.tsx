...
const BasisTuple = () => <span className="representation-atom">(<Vector name="p1" />, <Vector name="p2" />)</span>;
...
export function DiagonalizationPanel({ tab, analysis, input, onSwap, disabled = false }: {
  ...
  if (tab === 'basis') return basis ? <>
    <Formula><BasisName name="B" /> = <BasisTuple /></Formula>
    ...
    <button type="button" className="basis-fit-button" disabled={disabled} onClick={onSwap}>基底の1番目と2番目を交換</button>
    <p>固有ベクトルの基底の一例です。基底の取り方は唯一ではありません。順序を変えると座標も変わりますが、入力とその像は変わりません。</p>
    <Formula><Vector name="P" /> = <span className="representation-atom">[<Vector name="p1" />　<Vector name="p2" />]</span> = <Matrix values={basis.p} /></Formula>
  ...
  return <>
    ...
    {input.status === 'ready' ? <>
      ...
      <Formula><Vector name="u" /> = <BasisTuple /><Vector name="c" /> = <Vector name="P" /><Vector name="c" /> = <Column values={input.coordinates.inputViaCoordinates} /></Formula>
