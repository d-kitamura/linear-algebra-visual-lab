...
Linear Algebra Visual Labは、0〜3次元のベクトル空間、多項式、一次独立・一次従属、生成する空間、一次結合、基底・次元・座標、0〜3次元間の線形写像、表現行列・基底変換、固有値・対角化、内積・射影・正規直交基底を7つのLabで対話的に確認する教材アプリである。正規の公開URLは次のとおりである。

`https://d-kitamura.github.io/linear-algebra-visual-lab/`

## アプリが扱う情報

- 入力したベクトル、spanまたは基底候補の選択と順序、数ベクトル／多項式表示、一次結合ターゲット、比較用基底、線形写像の行列・2入力・スカラー、表現行列の両基底・変換方向、固有値／対角化の入力と基底列順、内積の種類・入力順・選択・直交化の段階、3Dカメラはブラウザ内で処理する。
...
- 数学的な判定には浮動小数点演算と許容誤差、および有理数による厳密演算を用途別に用いる。内積Labは入力されたbinary64値の正確な有理数で零残差を判定する一方、正規化・表示では数値検算を行い、保証できない結果を保留する。厳密な証明や成績評価の唯一の根拠にはせず、授業内の説明と併用する。
