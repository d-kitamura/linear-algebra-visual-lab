...
describe('フェーズ14詳細設計（D-127承認・14.4数2DのGS接続）', () => {
  it('14.4の確認待ちと未着手の14.5〜14.9を区別する', () => {
    ...
    expect(gsUnit.match(/- \[x\]/g)).toHaveLength(2);
    expect(gsUnit).toContain('- [ ] **確認ゲート:** D-129');
    expect(phase.split('### 14.5 ')[1]).not.toContain('- [x]');
