...
フェーズ 4「2D 一次結合エクスプローラ」までの実装・利用者確認・統合棚卸しと、共有URLのQR表示・PNG保存が完了しています。現在はフェーズ5「3D教材版」を進行中で、作業単位5.1としてLab上部の2D/3Dタブ、次元別の独立状態、共有URLによる初期次元選択、現在次元だけのResetを実装しました。3D側は固定表示とカメラを接続する5.2前のプレースホルダーです。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
