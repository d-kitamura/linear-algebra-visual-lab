...
利用者確認済み（D-093）。採用記号と対象範囲は[設計書](./REPRESENTATION_MATRIX_DESIGN.md)を参照する。11.3で数ベクトル2→2の画面へ接続した（D-094利用者確認済み）。11.4で1〜3次元の9次元組と3Dドラッグプレビューへ接続した（D-095利用者確認済み）。11.5で双方向の基底変換と往復検算を画面へ追加した（D-096利用者確認済み）。11.6では種別を呼出側に保持し、標準単項式基底の係数を同じAPIへ渡して多項式の表現行列・2経路・基底変換を導出する（D-097利用者確認済み）。11.7で共有から復元した基準係数も同じAPIで再計算する。派生値はURLに保存しない（D-098利用者確認待ち）。
...
上の数値例と失敗時の扱いはD-093として確認済み。11.3の2→2画面での確認項目はD-094を参照する。
