...
フェーズ6「共有と配布の仕上げ」とフェーズ8「複数Lab基盤と基底・次元Lab」の実装・利用者確認・統合棚卸しが完了しています。2D/3D切替、右手座標系・正投影カメラ、数値編集・解析・共有、rankに応じたspan幾何、一次結合、モードなしの矢先ドラッグ、平行・同一平面上への吸着、spanと係数幾何のドラッグ中更新を利用できます。直接操作の吸着距離は3Dを表示幅の3%、2Dを2%とし、通常ベクトルとターゲットは原点吸着を平行・同一平面吸着より優先します。現在は初版候補 `1.0.0-rc.1` で、授業実施までフェーズ7の評価を延期しています。基底・次元Labでは同じ状態を数ベクトルと多項式として読み替え、2Dを`\mathbb{R}[x]_1`、3Dを`\mathbb{R}[x]_2`へ対応させています。ベクトル空間Lab v3と独立した基底・次元Lab v1の共有URL・QR・Reset、3Dカメラ、20件の代表例、固定fixture、授業手順、Canvas代替導線、MIT License、利用・プライバシー案内を整備しました。シラバス第9〜10週へ接続するフェーズ9「線形写像Lab」を開始し、9.1で教育範囲、定義域／終域の連動画面、状態・共有境界をD-074として確定しました。9.2では行列積、核・像の基底、rank・nullity、次元定理、単射・全射を求める描画非依存APIを実装・自動検証済みです。現在はD-075の利用者確認待ちです。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
