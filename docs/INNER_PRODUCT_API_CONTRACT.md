...
2026-09-22更新。**D-125・D-126・D-127は利用者承認済み。14.3・14.4（D-128・D-129）承認済み、14.5数0D・1D・3D実装済み・D-130確認待ち。** 共有デコーダーは未実装（14.7予定）。14.2〜14.5の実装記録は第10〜13節を参照。実画面確認後の見直しを認める。
...
D-127は2026-09-21に承認済み。数2D画面は14.3、GS操作は14.4、数0〜3Dは14.5で接続した（第11〜13節）。次の14.6は積分内積と図の座標・逆変換ドラッグを一致させるため**「高」推奨**。
...
- 自動テストは状態・段階の純粋関数、SSRと接続ソース検査。14.4の実操作・390px・キーボードはD-129で利用者確認済み。14.5・D-130の利用者確認は別途行う。

## 13. 14.5数0D・1D・3D接続（D-130確認待ち）

2026-09-22。数ベクトルの4場面を実装。数学APIの判定・演算予算は不変。多項式と共有形式は未接続。
...
- 入力成分上限1e6、派生図上限1e6、数値失敗／WebGL失敗時も入力・解析・Resetが残る責務は維持。0D・1D・3Dの状態／SSR・純粋な幾何・共有3D接続を自動検証し、実ドラッグと狭幅はD-130で利用者が確認する。
