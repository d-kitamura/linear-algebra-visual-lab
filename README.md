...
フェーズ 4「2D 一次結合エクスプローラ」までの実装・利用者確認・統合棚卸しと、共有URLのQR表示・PNG保存が完了しています。現在はフェーズ5「3D教材版」を進行中です。5.1の2D/3Dタブと次元別状態は利用者確認済みで、5.2では右手座標系の固定3Dベクトル、補助格子、z軸上向きの正投影、回転・拡大・移動、視点プリセットを実装しました。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
