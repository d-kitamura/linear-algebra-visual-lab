...
フェーズ6「共有と配布の仕上げ」、フェーズ8「複数Lab基盤と基底・次元Lab」、フェーズ9「線形写像Lab」の実装・利用者確認・統合棚卸しが完了しています。2D/3D切替、右手座標系・正投影カメラ、数値編集・解析・共有、rankに応じたspan幾何、一次結合、基底・座標、多項式係数表示、定義域／終域連動、核・像、rank・nullity、線形性、次元定理を利用できます。現在は初版候補 `1.0.0-rc.1` で、授業実施までフェーズ7の評価を延期しています。3つのLabの共有状態、3Dカメラ、固定fixture、計28代表例、授業手順、Canvas代替導線、MIT License、利用・プライバシー案内を維持しています。フェーズ10「0・1次元空間の基礎拡張」は10.5まで利用者確認を終え、10.5で基底・次元Labも0D・1D・2D・3D切替へ拡張しました。1Dでは数直線上の基底候補・座標・比較基底と定数多項式空間`\mathbb{R}[x]_0`、0Dでは空の基底が満たす2条件と次元0を確認できます。10.6で線形写像Labにも0D・1Dを接続し、利用者確認待ちです。0D/1D共有URLは10.7で追加します。その後のフェーズ11には数ベクトルと多項式空間を扱う第四の「表現行列・基底変換Lab」を計画しています。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。

## 文書の役割

2026-09-05に10.5とD-086の利用者確認が完了しました。基底判定の対象空間と表示の不一致は10.5a・D-087で補修し、利用者確認も完了しました。現在の確認対象は10.6・D-088です。数ベクトル／多項式の空間全体を対象に、判定・基底例・座標・比較用基底を統一しました。現在地と確認範囲は [`docs/INVENTORY_2026-09-05.md`](./docs/INVENTORY_2026-09-05.md) を参照してください。
