...
フェーズ6「共有と配布の仕上げ」までの実装・利用者確認・統合棚卸しが完了しています。2D/3D切替、右手座標系・正投影カメラ、数値編集・解析・共有、rankに応じたspan幾何、一次結合、モードなしの矢先ドラッグ、平行・同一平面上への吸着、spanと係数幾何のドラッグ中更新を利用できます。3Dの直接操作では、通常ベクトルと一次結合ターゲットの吸着距離を表示幅の3%とし、ターゲットは選択集合が生成する原点・直線・平面へ吸着します。2Dの吸着距離は従来どおり表示幅の1%です。2D・3D合わせて16件の代表例、現行v3の本番共有URL、QR表示・PNG保存、授業手順、Canvasを利用できない場合の代替導線、MIT License、利用・プライバシー案内も整備済みです。現在は初版候補 `1.0.0-rc.1` です。授業実施までフェーズ7の評価を延期し、フェーズ8「複数Lab基盤と基底・次元Lab」を進めています。8.3の数学ロジックと表記は利用者確認済みです。8.4では共通メニューから選べる2D/3D基底エクスプローラを追加し、順序付き候補、一次独立性、生成条件、次元、rank、基底の一例をベクトル編集とspan幾何へ接続しました。現在は8.4の利用者確認待ちで、座標エクスプローラは8.5、基底・次元Labの共有URL・QRは8.7で追加します。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
