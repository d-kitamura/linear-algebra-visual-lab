...
フェーズ 4「2D 一次結合エクスプローラ」までの実装・利用者確認・統合棚卸しと、共有URLのQR表示・PNG保存が完了しています。現在はフェーズ5「3D教材版」を進行中です。5.1〜5.4で次元切替、右手座標系・正投影カメラ、3D数値編集・解析・共有、rank 0〜3のspan幾何表示まで利用者確認済みです。5.5では3Dターゲットの数値入力、不能・一意・不定の係数表示、2項の平行四辺形、3項の平行六面体を追加しました。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
