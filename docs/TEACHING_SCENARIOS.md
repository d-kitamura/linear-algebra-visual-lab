...
フェーズ4では一次結合の数学・共有・教材表現を2Dで確立した。D-037により、フェーズ5は右手座標系・z軸上向きの正投影、カメラ共有、数値編集先行、操作モードと軸拘束による直接編集試作を採用した。5.1では次元切替と状態境界を実装し、5.2以降で3D描画、カメラ操作、spanが直線・平面・3次元空間になる場合の表現を接続する。
