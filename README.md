...
線形代数を学ぶ学生が、ベクトル、一次独立・一次従属、生成する空間、基底・次元、線形写像をブラウザ上で視覚的・対話的に理解するための教材アプリです。3つのLabが0〜3次元に対応し、線形写像Labでは定義域・終域の全16次元組を扱えます。

フェーズ6「共有と配布の仕上げ」、フェーズ8「複数Lab基盤と基底・次元Lab」、フェーズ9「線形写像Lab」の実装・利用者確認・統合棚卸しが完了しています。2D/3D切替、右手座標系・正投影カメラ、数値編集・解析・共有、rankに応じたspan幾何、一次結合、基底・座標、多項式係数表示、定義域／終域連動、核・像、rank・nullity、線形性、次元定理を利用できます。現在は初版候補 `1.0.0-rc.1` で、授業実施までフェーズ7の評価を延期しています。3つのLabの共有状態、3Dカメラ、固定fixture、既存28例と低次元13例（計41代表例）、授業手順、Canvas代替導線、MIT License、利用・プライバシー案内を維持しています。フェーズ10「0・1次元空間の基礎拡張」は10.5まで利用者確認を終え、10.5で基底・次元Labも0D・1D・2D・3D切替へ拡張しました。1Dでは数直線上の基底候補・座標・比較基底と定数多項式空間`\mathbb{R}[x]_0`、0Dでは空の基底が満たす2条件と次元0を確認できます。10.6で線形写像Labにも0D・1Dを接続し、10.7で3Labの共有URL・QR・共有時Resetと13教材例を追加しました。10.7の確認ゲートはすべて利用者確認済みです。10.8の統合棚卸し・補修も利用者確認を終え、フェーズ10は完了しました。11.1の記号・状態設計はD-092として利用者確認済みで完了し、11.2・D-093も利用者確認済みです。11.3で第四Labの数ベクトル2→2画面を追加し、D-094として利用者確認済みです。11.4で数ベクトル1D〜3Dの9次元組、次元組別状態、両図の表示範囲・3Dカメラ、リアルタイムの矢先編集へ拡張しました（D-095利用者確認済み・11.4完了）。11.5で座標の2経路、独立した基底変換モード、双方向変換と往復検算、標準・順序変更・斜交基底の例を追加しました（D-096利用者確認済み）。11.6で両側の数ベクトル／多項式の選択、1〜3次元係数空間、多項式と座標の区別、微分・x倍・平行移動の例を追加しました（D-097利用者確認済み）。11.7で第四Labの共有URL・QR・共有時Resetを追加しました（D-098利用者確認済み）。11.8の授業用11例・観察課題・期待値は共有URL付きMarkdownへ集約しました。D-100により4つの補助パネルを削除し、アプリは操作と数式表示に集中させています（利用者確認待ち）。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。

## 文書の役割

2026-09-05に10.5とD-086の利用者確認が完了しました。基底判定の対象空間と表示の不一致は10.5a・D-087で補修し、利用者確認も完了しました。10.8・D-091は利用者確認済みです。11.1・D-092は承認済みで、数学表記ルールへ反映しました。11.2の数学APIは利用者確認済みです。11.7まで利用者確認済み、11.8は実装済み・利用者確認待ちです。1D〜3Dの9次元組、基底・入力のドラッグと数値編集、順序変更、全幅4タブを確認できます。現在のモード・次元組だけをResetし、次元組を切り替えても写像・両基底・入力と表示状態を保持します。新Labも現在の場面の写像・基底順・入力・種別・方向・3DカメラをURL・QRで共有できます。数ベクトル／多項式の空間全体を対象に、判定・基底例・座標・比較用基底を統一しました。現在地と確認範囲は [`docs/PROJECT_STATUS.md`](./docs/PROJECT_STATUS.md)、過去の棚卸しは [`docs/INVENTORY_2026-09-05.md`](./docs/INVENTORY_2026-09-05.md) を参照してください。
