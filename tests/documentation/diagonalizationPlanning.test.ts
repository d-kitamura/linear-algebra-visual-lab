...
describe('フェーズ13の未承認設計と利用者指定の範囲', () => {
  it('13.1の承認と13.2数学APIの確認待ちを区別する', () => {
    ...
    expect(implementation).toContain('利用者確認待ち（D-117）');
    expect(implementation.match(/- \[x\]/g)).toHaveLength(2);
    expect(implementation).toContain('- [ ] **確認ゲート:**');
    expect(phase13.split('### 13.3 ')[1]).not.toContain('- [x]');
