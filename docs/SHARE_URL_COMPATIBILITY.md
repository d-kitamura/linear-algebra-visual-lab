...
- 4形式は最初の正式リリースで保証対象になる候補として固定fixtureを用意している。
...
1. 現行の`vector-space` v4、`basis-dimension` v2、`linear-map` v2、`representation-matrix` v1を最初の保証対象候補とし（対象版は正式リリース時に再確認）、正式承認後は本アプリを提供している期間、固定の年数を設けず読み込みを維持する。
