...
フェーズ6「共有と配布の仕上げ」とフェーズ8「複数Lab基盤と基底・次元Lab」の実装・利用者確認・統合棚卸しが完了しています。2D/3D切替、右手座標系・正投影カメラ、数値編集・解析・共有、rankに応じたspan幾何、一次結合、モードなしの矢先ドラッグ、平行・同一平面上への吸着、spanと係数幾何のドラッグ中更新を利用できます。現在は初版候補 `1.0.0-rc.1` で、授業実施までフェーズ7の評価を延期しています。ベクトル空間Labと基底・次元Labに加え、シラバス第9〜10週へ接続するフェーズ9「線形写像Lab」を実装中です。9.1で教育範囲と状態境界、9.2で行列積、核・像の基底、rank・nullity、次元定理、単射・全射の描画非依存APIを確定しました。9.3では`T: \mathbb{R}^2→\mathbb{R}^2`の定義域／終域連動画面、入力ベクトルと行列の編集、標準基底の像、6種の代表例、切替可能な格子の像を追加し、現在はD-076の利用者確認待ちです。既存2Labの共有URL・QR・Reset、3Dカメラ、代表例、固定fixture、授業手順、Canvas代替導線、MIT License、利用・プライバシー案内も維持しています。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。
