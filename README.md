...
フェーズ 3「2D 教材版の完成」まで実装・利用者確認・統合棚卸しが完了しています。現在は新設したフェーズ 4「2D 一次結合エクスプローラ」を進めており、4.1としてターゲットを一次結合で表現できるか、その係数が一意か無数かを解析する2D/3D共通ロジックを実装しました。公開版は [Linear Algebra Visual Lab](https://d-kitamura.github.io/linear-algebra-visual-lab/) です。3D教材版はフェーズ 5で、この仕組みも含めて実装します。
